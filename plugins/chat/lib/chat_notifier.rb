...
class Chat::ChatNotifier
  ...
  def notify_creator_of_inaccessible_mentions(to_notify)
    mentions = to_notify.extract!(:unreachable, :welcome_to_join, :too_many_members, :group_mentions_disabled)
    return if mentions.values.all?(&:blank?)

    warnings = [
      inaccessible_mention_payload(:cannot_see, mentions[:unreachable]) { |user| user.username },
      inaccessible_mention_payload(:without_membership, mentions[:welcome_to_join], include_ids: true) { |user| user.username },
      inaccessible_mention_payload(:too_many_members, mentions[:too_many_members]) { |group| group.name },
      inaccessible_mention_payload(:group_mentions_disabled, mentions[:group_mentions_disabled]) { |group| group.name },
    ].compact
