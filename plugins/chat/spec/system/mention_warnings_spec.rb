    ...
    context "when mentioning a group" do
      context "when the group doesn’t allow mentions" do
        ...
        it "displays a warning" do
          ...
          expect(
            page.find(".chat-mention-warnings-list__simple")
          ).to have_content(admin_mentionable_group.name)
        end
      end

      context "when the group allow mentions" do
        fab!(:publicly_mentionable_group) { Fabricate(:group, mentionable_level: Group::ALIAS_LEVELS[:everyone]) }
        ...
        context "when the group has too many members" do
          ...
          it "displays a warning" do
            ...
            expect(
              page.find(".chat-mention-warnings-list__simple")
            ).to have_content(publicly_mentionable_group.name)
          end
        end
