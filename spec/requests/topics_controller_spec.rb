      ...
      context "when key scope has restricted params" do
        before do
          ApiKeyScope.create(
            ...
            allowed_parameters: { "category_id" => ["#{topic.category_id}"] }
          )
        end

        it "fails to update topic status in an unpermitted category" do
          put "/t/#{topic.id}/status.json",
            params: { status: "closed", enabled: "true", category_id: tracked_category.id },
            headers: { "HTTP_API_KEY" => api_key.key, "HTTP_API_USERNAME" => api_key.user.username }
        ...
        it "updates topic status in a permitted category" do
          put "/t/#{topic.id}/status.json",
            params: { status: "closed", enabled: "true", category_id: topic.category_id },
            headers: { "HTTP_API_KEY" => api_key.key, "HTTP_API_USERNAME" => api_key.user.username }
