...
describe BackupRestoreV2::Database do
  ...
  describe ".core_migration_files" do
    it "returns regular and post_migrate migrations" do
      ...
      expect(files).to include(absolute_path("db/post_migrate/20210528003603_fix_badge_image_avatar_upload_security_and_acls.rb"))
    end

    it "doesn't returns plugin migrations" do
      files = described_class.core_migration_files
      expect(files).to_not include(absolute_path("plugins/poll/db/migrate/20180820073549_create_polls_tables.rb"))
    end
  end
  ...
  describe ".current_core_migration_version" do
    ...
    it "returns the max schema migration version" do
      ...
      expect(described_class.current_core_migration_version).to eq(20130213203300)
    end
    ...
    it "returns the max schema migration version from a post migration" do
      ...
      expect(described_class.current_core_migration_version).to eq(20211201221028)
    end
