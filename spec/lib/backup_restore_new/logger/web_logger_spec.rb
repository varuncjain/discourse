...
describe BackupRestoreV2::Logger::WebLogger do
  ...
  def expect_publish_on_message_bus(*messages)
    published_messages = freeze_time("2022-10-13T21:20:17Z") do
      MessageBus.track_publish { yield }
    end
  ...
  describe "#log_event" do
    it "publishes a message" do
      expect_publish_on_message_bus("Foo") do
  ...
  describe "#log_step" do
    context "without progress" do
      ...
      it "yields control" do
        expect { |block| subject.log_step("Step 3: Payload Separation", &block) }
  ...
  describe "#log" do
    it "publishes a message" do
      expect_publish_on_message_bus("Foo bar") do
        subject.log("Foo bar")
      end
    end
  end
