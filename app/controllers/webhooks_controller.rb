...
class WebhooksController < ActionController::Base
  ...
  def handle_mailgun_legacy(params)
    return signature_failure unless valid_mailgun_signature?(params["token"], params["timestamp"], params["signature"])
  ...
  def handle_mailgun_new(params)
    ...
    return signature_failure unless valid_mailgun_signature?(signature["token"], signature["timestamp"], signature["signature"])
  ...
  def valid_mandrill_signature?
    ...
    params.permit(:mandrill_events).to_h.sort_by(&:first).each do |key, value|
      payload += key.to_s
      payload += value
    end

    payload_signature = OpenSSL::HMAC.digest('sha1', SiteSetting.mandrill_authentication_key, payload)
    ActiveSupport::SecurityUtils.secure_compare(signature, Base64.strict_encode64(payload_signature))
  end
